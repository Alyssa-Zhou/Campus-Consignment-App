...
public class UserDetailController extends BaseController {
    ...
    public JsonResult<String> changeStudentCard(@RequestParam("file") MultipartFile file, HttpSession session) {
        ...
        // 获取当前项目的绝对磁盘路径
        String parent = session.getServletContext().getRealPath("upload/student");
        ...
        // 头像路径
        String studentcard = "/upload/student/" + filename;
        // 从Session中获取uid和username
        String id = getIdFromSession(session);
        // 将头像写入到数据库中
        userDetailService.changeStudentCard(id, studentcard);

        // 返回成功头像路径
        return new JsonResult<String>(OK, studentcard);
    }
}
